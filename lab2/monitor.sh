...
function init
{
	...
	TIME_INTERVAL=${@:$#} #Time interval is the last argument (in "$@", the para item on "$#" postion)
...
function jiffies_to_percentage {
	...
	diff_stime=$(expr $new_stime-$old_stime)
	diff_utime=$(expr $new_utime-$old_utime)
...
function generate_report {
	...
	numFile=$(ls ./reports_dir | wc -w)
	let numFile=numFile+1
	if [ $numFile -gt $MAXIMUM_REPORTS ]; then
...
function calculate_mem_usage
{
	...
	mem_usage=$(grep -n VmRSS: /proc/$PID/status | awk '{print $2}')
...
function notify
{
	...
	if [ $memFlag -eq 1 ]; then
		if [ $cpu_usage_int -gt $CPU_THRESHOLD ] || [ $mem_usage -gt $MEM_THRESHOLD ]; then
			/usr/bin/mailx -s "Process Error Message" $USER <./reports_dir/$(ls -t ./reports_dir/ | head -1)
			# echo "email sent!"
		fi
	else
